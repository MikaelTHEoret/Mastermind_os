...
export class MemoryManager {
  ...
  private constructor() {
    ...
    // Initialize store with complete config
  ...
  private async initializeAIProvider() {
    ...
    try {
      this.aiProvider = await createAIProvider(appConfig.ai, { 
        enableMemory: false, // Disable memory features
      ...
      this.aiProvider = {
        ...
        generateEmbedding: async () => new Array(0).fill(0), // Return empty array for embeddings
        initialize: async () => {},
        cleanup: async () => {}
      };
    }
  ...
  private async calculateImportance(memory: MemoryEntry): Promise<number> {
    ...
    score += Math.min(accessCount / 100, 0.5); // Increased weight since we removed embeddings

    // Factor 2: Association count
    const associationCount = memory.associations?.length || 0;
    score += Math.min(associationCount / 10, 0.5); // Increased weight since we removed embeddings
...
Summary:`;
  ...
  async retrieveRelevantMemories(
    ...
    this.ensureInitialized();
    try {
      // Simple keyword-based search when embeddings are disabled
      return await this.store.search({
        text: context,
        limit,
        minRelevance: 0.3 // Lower threshold for keyword matching
      });
    } catch (error) {
