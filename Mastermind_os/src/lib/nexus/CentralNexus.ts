...
class CentralNexusSystem {
  ...
  public async initialize(): Promise<void> {
    ...
    try {
      ...
      try {
        // Initialize storage and memory systems first
        await Promise.all([
          storageManager.initialize().catch(error => {
            this.logger.addLog({
              source: 'CentralNexus',
              type: 'warning',
              message: `Storage initialization warning: ${error}. System will continue with limited functionality.`
            });
          }),
          memoryManager.initialize().catch(error => {
            this.logger.addLog({
              source: 'CentralNexus',
              type: 'warning',
              message: `Memory initialization warning: ${error}. System will continue with limited functionality.`
            });
          })
        ]);

        // Then register core agents
        await this.registerCoreAgents();
        this.logger.addLog({
          source: 'CentralNexus',
          type: 'info',
