...
start "Mastermind Codex OS v2 - Backend Server" cmd /c "%~dp0start-continuous-server.bat"
...
if exist "%NGROK_PATH%" (
    ...
    if "%NGROK_AUTH_TOKEN%"=="" (
        start "Mastermind Codex OS v2 - Ngrok" cmd /c "%~dp0start-continuous-ngrok.bat" --ngrok-path "%NGROK_PATH%"
    ) else (
        echo Using provided ngrok auth token for API access
        start "Mastermind Codex OS v2 - Ngrok" cmd /c "%~dp0start-continuous-ngrok.bat" --ngrok-path "%NGROK_PATH%" --auth-token "%NGROK_AUTH_TOKEN%"
    )
) else (
...
echo - If ngrok is running, you can find the public URL in backend/ngrok_url.txt
echo.
